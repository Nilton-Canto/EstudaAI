...
from django.contrib.auth.admin import UserAdmin #importa a classe UserAdmin, que já traz toda a configuração padrão da interface de administração do Django para o modelo User.
from .models import Usuario #mporta o modelo customizado de usuário (Usuario) definido em models.py.

@admin.register(Usuario) #registra o modelo Usuario no admin do Django.
class UsuarioAdmin(UserAdmin): #cria uma classe personalizada para controlar como o modelo Usuario será exibido no admin. Ela herda de UserAdmin.
    fieldsets = UserAdmin.fieldsets + (
        ('Informações acadêmicas', {'fields': ('nome', 'universidade', 'curso', 'ano_formatura', 'idade')}),
