...
def admin_dashboard(request):
    ...
    ultimas_trilhas = Trilha.objects.select_related('usuario', 'area').order_by('-data_criacao')[:5]
...
def admin_trilha_create(request):
    ...
    return render(request, 'usuarios/admin/trilha_form.html', {
        'usuarios': usuarios,
        'areas': areas
    })
    ...
    if request.method == 'POST':
        ...
        if not titulo:
            ...
            try:
                ...
                messages.success(request, f'✅ Trilha "{trilha.titulo}" atualizada com sucesso!')
                return redirect('admin_trilhas_list')
