...
def gerar_trilha_curso(request):
    ...
    """
    logger.info(
        f"Requisição de trilha de curso recebida: {request.method} {request.path}"
    )
    ...
    try:
        ...
        if (
            ...
            or settings.GEMINI_API_KEY == "test_key_for_verification"
        ):
        ...
        if user_id:
            try:
                ...
                logger.info(
                    f"Dados do usuário {usuario.username} carregados para personalização"
                )
...
def chat_with_gemini(request):
    ...
    try:
        ...
        if not message:
            ...
            return Response(
                {"error": "Mensagem é obrigatória"}, status=status.HTTP_400_BAD_REQUEST
            )
...
def health_check(request):
    ...
    gemini_configured = bool(
        ...
        and settings.GEMINI_API_KEY != "test_key_for_verification"
    )
