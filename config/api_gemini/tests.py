...
class APITrilhaTest(APITestCase):
    ...
    def test_gerar_trilha_sucesso(self, mock_model):
        ...
        data = {
            'solicitacao': 'Quero aprender Python',
    ...
    def test_gerar_trilha_com_dados_usuario(self, mock_model):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['trilha']['titulo'], 'Python para Ciência da Computação')
    ...
    def test_gerar_trilha_erro_gemini(self, mock_model):
        ...
        # Mock de erro na API
        mock_model.return_value.generate_content.side_effect = Exception("Erro na API do Gemini")
    ...
    def test_listar_trilhas_usuario(self):
        ...
        url = reverse('api_gemini:listar_trilhas_usuario', kwargs={'user_id': self.user.id})
...
class ChatAPITest(APITestCase):
    ...
    def test_chat_sucesso(self, mock_model):
        ...
        self.assertEqual(response.data['response'], 'Esta é uma resposta de teste do Gemini')
...
class IntegrationTestCase(APITestCase):
    ...
    def test_endpoints_existem(self):
        ...
        response = self.client.post(self.trilha_url, data={'solicitacao': 'teste'}, format='json')
        ...
        response = self.client.post(self.chat_url, data={'message': 'teste'}, format='json')
        self.assertIn(response.status_code, [
            status.HTTP_200_OK, 
            status.HTTP_400_BAD_REQUEST, 
            status.HTTP_500_INTERNAL_SERVER_ERROR
    ...
    def test_validacao_entrada_consistente(self):
        ...
        self.assertIn(response_trilha.status_code, [
            status.HTTP_400_BAD_REQUEST, 
            status.HTTP_500_INTERNAL_SERVER_ERROR
        ])
        self.assertIn(response_chat.status_code, [
            status.HTTP_400_BAD_REQUEST, 
            status.HTTP_500_INTERNAL_SERVER_ERROR
        ])